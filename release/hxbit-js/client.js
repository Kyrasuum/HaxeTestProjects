...
$hxClasses["User"] = User;
User.__name__ = "User";
User.__interfaces__ = [hxbit_NetworkSerializable];
User.prototype = {
	...
	,alive: function() {
		this.init();
		if(this.uid == this.client.uid) {
			this.client.user = this;
			this.client.host.self.ownerObject = this;
		}
	}
	,init: function() {
		this.client = Main.inst;
		this.client.log("Init " + Std.string(this),{ fileName : "src/Main.hx", lineNumber : 26, className : "User", methodName : "init"});
	...
	,networkRPC: function(__ctx,__id,__clientResult) {
		throw haxe_Exception.thrown("Unknown RPC identifier " + __id);
	}
	...
	,getSerializeSchema: function() {
		...
		schema.isFinal = hxbit_Serializer.isClassFinal(User.__clid);
		return schema;
	}
...
Main.prototype = $extend(hxd_App.prototype,{
	init: function() {
		...
		this.host.setLogger(function(msg) {
			_gthis.log(msg,{ fileName : "src/Main.hx", lineNumber : 50, className : "Main", methodName : "init"});
		});
		this.log("Server not allowed on this platform",{ fileName : "src/Main.hx", lineNumber : 57, className : "Main", methodName : "init"});
		try {
			this.host.wait(Main.HOST,Main.PORT,function(c) {
				_gthis.log("Client Connected",{ fileName : "src/Main.hx", lineNumber : 63, className : "Main", methodName : "init"});
			});
			this.host.onMessage = function(c,uid) {
				_gthis.log("Client identified (" + uid + ")",{ fileName : "src/Main.hx", lineNumber : 66, className : "Main", methodName : "init"});
				var client = new User(uid);
				c.ownerObject = client;
				c.sync();
			};
			this.log("Server Started",{ fileName : "src/Main.hx", lineNumber : 71, className : "Main", methodName : "init"});
			this.start();
		} catch( _g ) {
			this.log("Connecting",{ fileName : "src/Main.hx", lineNumber : 76, className : "Main", methodName : "init"});
			this.uid = 1 + Std.random(1000);
			this.host.connect(Main.HOST,Main.PORT,function(b) {
				if(!b) {
					_gthis.log("Failed to connect to server",{ fileName : "src/Main.hx", lineNumber : 81, className : "Main", methodName : "init"});
					return;
				}
				_gthis.log("Connected to server",{ fileName : "src/Main.hx", lineNumber : 84, className : "Main", methodName : "init"});
...
hxd_net_Socket.prototype = {
	...
	,onError: function(msg) {
		throw haxe_Exception.thrown("Socket Error " + msg);
	}
	,onData: function() {
	}
...
var hxd_net_SocketClient = function(host,s) {
	...
	if(s != null) {
		s.onData = function() {
			while(_gthis.socket != null && _gthis.readData(_gthis.socket.input,_gthis.socket.input.get_available())) {
			}
		};
	}
};
...
User.__clid = hxbit_Serializer.registerClass(User);
Main.HOST = "127.0.0.1";
Main.PORT = 6676;
h3d_Buffer.GUID = 0;
